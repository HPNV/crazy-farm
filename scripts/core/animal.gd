...
@export var stats: AnimalStat

@onready var movement_component: ActorMovementComponent = $MovementComponent
@onready var wander_component: WanderComponent = $WanderComponent
...
func _physics_process(_delta: float) -> void:
	if movement_component == null or wander_component == null:
		return

	var direction = wander_component.get_direction()
	...
	if animation_component != null:
		...
		animation_component.face_direction_x(direction.x)
